...
# Gera JAR Desktop + JAR Servidor Mobile (nao monoliticos)
...
if (Test-Path "configuracao_banco.json") {
    Copy-Item "configuracao_banco.json" $outputDir -Force
    Write-Host "  - configuracao_banco.json copiado" -ForegroundColor Green
} else {
    Write-Host "  [AVISO] configuracao_banco.json nao encontrado" -ForegroundColor Yellow
}
