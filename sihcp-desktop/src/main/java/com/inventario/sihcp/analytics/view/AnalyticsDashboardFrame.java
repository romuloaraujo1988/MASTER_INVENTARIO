...
import com.inventario.sihcp.analytics.model.AnalyticsKPIs;
 ...
 */
public class AnalyticsDashboardFrame extends JFrame implements DashboardObserver {
    ...
    // Timer removido em favor de reatividade via EventBus
    ...
    public AnalyticsDashboardFrame() {
        ...
        // Registrar no barramento de eventos
    ...
    private void initComponents() {
        ...
        // Configurar atualização automática - REMOVIDO: Agora usa EventBus
    ...
    private JPanel criarPainelSuperior() {
        ...
        JCheckBox chkAutoRefresh = new JCheckBox("Reatividade Real-time ativada");
        chkAutoRefresh.setSelected(true);
        chkAutoRefresh.setEnabled(false); // Sempre ativo agora
    ...
    public void onDashboardEvent(DashboardEvent event) {
        logger.info("Evento recebido no Analytics: {}", event.getType());
        // Atualizar se for um evento relevante
        switch (event.getType()) {
            case PATRIMONIO_ATUALIZADO:
            case COLETA_SINCRONIZADA:
            case INVENTARIO_ALTERADO:
                SwingUtilities.invokeLater(this::atualizarKPIs);
                break;
            default:
                break;
        }
