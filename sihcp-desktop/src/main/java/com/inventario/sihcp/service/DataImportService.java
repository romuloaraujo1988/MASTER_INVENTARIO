...
public class DataImportService {
    ...
    public ImportResult importarTodosDados(ProgressListener listener) {
        ...
        try {
            ...
            try {
                com.inventario.sihcp.offline.SQLiteConnection sqliteConn = com.inventario.sihcp.offline.SQLiteConnection.getInstance();
            ...
            // 0. Setores (IMPORTANTE: Importar antes de salas e patrimônios)
            System.out.println("\n>>> ETAPA 0: Importando setores...");
            listener.onProgress("Importando setores...", 3);
            result.setores = importarSetores(listener);
            listener.onProgress("Setores importados: " + result.setores, 5);
            ...
            System.out.println("Setores: " + result.setores);
            ...
            LOGGER.info(String.format("Importação concluída: %d setores, %d patrimônios, %d salas, %d responsáveis, %d usuários",
                result.setores, result.patrimonios, result.salas, result.responsaveis, result.usuarios));
    ...
    /**
     * Importa setores do servidor para SQLite
     */
    private int importarSetores(ProgressListener listener) throws SQLException {
        LOGGER.info("Importando setores do servidor PostgreSQL");
        
        try {
            // Buscar todos os setores do PostgreSQL
            var setores = setorDAO.findAll("NOME");
            
            LOGGER.info("Total de setores encontrados: " + setores.size());
            
            if (setores.isEmpty()) {
                LOGGER.warning("Nenhum setor encontrado no servidor!");
                return 0;
            }
            
            // Limpar tabela antes de importar
            System.out.println(">>> Limpando tabela local_setor...");
            try (Connection conn = offlineDAO.getConnection()) {
                try (var stmt = conn.createStatement()) {
                    int deleted = stmt.executeUpdate("DELETE FROM local_setor");
                    System.out.println(">>> ✅ Tabela local_setor limpa - " + deleted + " registros removidos");
                }
            }
            ...
            for (var setor : setores) {
                try {
                    var setorMap = new java.util.HashMap<String, Object>();
                    setorMap.put("id", setor.getId());
                    setorMap.put("nome", setor.getNome());
                    setorMap.put("descricao", setor.getDescricao());
                    setorMap.put("responsavel", setor.getResponsavelSetor());
                    setorMap.put("telefone", setor.getTelefone());
                    setorMap.put("email", setor.getEmail());
                    setorMap.put("ativo", setor.getAtivo() != null ? setor.getAtivo() : true);
                    
                    offlineDAO.salvarSetor(setorMap);
                    imported++;
                } catch (Exception e) {
                    LOGGER.log(Level.WARNING, "Erro ao importar setor ID " + setor.getId(), e);
                }
            }
            
            LOGGER.info("Setores importados com sucesso: " + imported);
            return imported;
            
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Erro ao importar setores", e);
            throw new SQLException("Erro ao importar setores: " + e.getMessage(), e);
        }
    }
    ...
    private int importarPatrimonios(ProgressListener listener) throws SQLException {
        ...
        try {
            ...
            try (Connection conn = com.inventario.sihcp.offline.SQLiteConnection.getInstance().getConnection()) {
                try (var stmt = conn.createStatement()) {
        ...
        try {
            ...
            for (var sala : salas) {
                try {
                    ...
                    salaMap.put("id_setor", sala.getIdSetor());
