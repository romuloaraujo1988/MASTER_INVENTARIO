...
public class OfflineDAO {
     ...
     */
    public int limparColetasPendentesJaSincronizadas(com.inventario.sihcp.dao.ColetaDAO coletaDAO) {
    ...
    public int salvarSala(Map<String, Object> sala) throws SQLException {
        String sql = """
            ...
            (id, nome, descricao, bloco, andar, capacidade, tipo, ativa, id_setor)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """;
        ...
        try (Connection conn = sqliteConnection.getConnection();
            ...
            // ID Setor (pode ser NULL)
            stmt.setObject(9, sala.get("id_setor"));
    ...
    /**
     * Salva um setor no banco local
     * @param setor Dados do setor
     * @return ID gerado
     * @throws SQLException
     */
    public int salvarSetor(Map<String, Object> setor) throws SQLException {
        String sql = """
            INSERT OR REPLACE INTO local_setor 
            (id, nome, descricao, responsavel, telefone, email, ativo, sync_status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """;
        ...
        try (Connection conn = sqliteConnection.getConnection();
            ...
            stmt.setString(4, (String) setor.get("responsavel"));
            stmt.setString(5, (String) setor.get("telefone"));
            stmt.setString(6, (String) setor.get("email"));
            stmt.setBoolean(7, (Boolean) setor.getOrDefault("ativo", true));
            stmt.setString(8, "SYNCED"); // Dados importados já estão sincronizados
            
            int rowsAffected = stmt.executeUpdate();
            
            if (rowsAffected > 0) {
                LOGGER.fine(() -> "Setor salvo offline - ID: " + setor.get("id"));
                return (Integer) setor.get("id");
            }
            
            throw new SQLException("Falha ao salvar setor offline");
            
        } catch (SQLException e) {
            System.err.println(">>> ❌ ERRO ao salvar setor no SQLite: " + e.getMessage());
            LOGGER.log(Level.SEVERE, "Erro ao salvar setor offline", e);
            throw e;
        }
    }
    ...
    public List<Map<String, Object>> listarSetores() throws SQLException {
        String sql = "SELECT * FROM local_setor WHERE ativo = 1 ORDER BY nome";
        ...
        try (Connection conn = sqliteConnection.getConnection();
            ...
            LOGGER.log(Level.SEVERE, "Erro ao listar setores offline", e);
            throw e;
        }
    }
