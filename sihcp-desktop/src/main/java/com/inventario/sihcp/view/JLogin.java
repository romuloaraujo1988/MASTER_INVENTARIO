...
public class JLogin extends JFrame {
    private static final Logger logger = LoggerFactory.getLogger(JLogin.class);
    ...
    private com.inventario.sihcp.service.UnifiedAuthService unifiedAuthService;
    ...
    public JLogin() {
        ...
        try {
            unifiedAuthService = new com.inventario.sihcp.service.UnifiedAuthService();
    ...
    private JPanel createFooterPanel() {
        ...
        btnConfig.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 11));
    ...
    private void abrirConfiguracaoBanco() {
        com.inventario.sihcp.util.ConfiguracaoBancoUtil.abrirDialogConfiguracao(this);
    }
    ...
    private void ativarModoOffline() {
        if (unifiedAuthService != null) {
            logger.info("DEBUG: Ativando modo OFFLINE_ONLY via checkbox");
            unifiedAuthService.setAuthMode(com.inventario.sihcp.service.UnifiedAuthService.AuthMode.OFFLINE_ONLY);
            atualizarIndicadorModoOffline();
            logger.info("DEBUG: Modo offline ativado. isOperatingOffline = {}", unifiedAuthService.isOperatingOffline());
        }
    }
    ...
    private void desativarModoOffline() {
        if (unifiedAuthService != null) {
            logger.info("DEBUG: Desativando modo offline, voltando para AUTO");
            unifiedAuthService.setAuthMode(com.inventario.sihcp.service.UnifiedAuthService.AuthMode.AUTO);
            atualizarIndicadorModoOffline();
            logger.info("DEBUG: Modo AUTO ativado. isOperatingOffline = {}", unifiedAuthService.isOperatingOffline());
        }
    }
    ...
    private void performLogin() {
        ...
        try {
            ...
            if (forcarOffline) {
                logger.info("DEBUG: Forçando modo OFFLINE_ONLY");
                unifiedAuthService.setAuthMode(com.inventario.sihcp.service.UnifiedAuthService.AuthMode.OFFLINE_ONLY);
            } else {
                logger.info("DEBUG: Usando modo AUTO");
                unifiedAuthService.setAuthMode(com.inventario.sihcp.service.UnifiedAuthService.AuthMode.AUTO);
            }
            
            // Verificar o modo atual antes de autenticar
            logger.info("DEBUG: Modo atual antes de autenticar: {}", 
                (unifiedAuthService.isOperatingOffline() ? "OFFLINE" : "ONLINE"));
            
            // Autenticar usando o serviço unificado (online + offline)
            com.inventario.sihcp.service.UnifiedAuthService.AuthResult resultado = unifiedAuthService.autenticar(usuario, senha);
            ...
            if (resultado.success && resultado.usuario != null) {
                ...
                logger.info("DEBUG: Login bem-sucedido{}", modoAuth);
    ...
    public static void main(String args[]) {
        ...
        if (!com.inventario.sihcp.util.SingleInstanceLock.tryLock()) {
            // Outra instância já está rodando
            com.inventario.sihcp.util.SingleInstanceLock.showInstanceAlreadyRunningMessage();
            System.exit(0);
            return;
        }
