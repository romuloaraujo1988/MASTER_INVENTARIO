...
import com.inventario.sihcp.service.ServiceFactory;
import com.inventario.sihcp.service.ResponsavelService;
import com.inventario.sihcp.service.SetorService;
...
public class InventarioFormDialog extends JDialog {
    ...
    private void initComponents() {
        ...
        btnSalvar = ButtonStyleFactory.createPrimaryButton(textoBotao);
        btnCancelar = ButtonStyleFactory.createSecondaryButton("Cancelar");
    ...
    private JPanel criarAbaParticipantes() {
        ...
        // Botão adicionar
        btnAdicionarParticipante = ButtonStyleFactory.createSmallSuccessButton("Adicionar →");
        ...
        // Botão remover
        btnRemoverParticipante = ButtonStyleFactory.createSmallDangerButton("← Remover");
    ...
    private void salvarInventario() {
        try {
            ...
            // Validar configurações de setores e participantes antes de criar no BD
            if (!validarConfiguracaoSetores()) {
                return; // O método de validação já exibe o alerta
            }
            if (!validarConfiguracaoParticipantes()) {
                return; // O método de validação já exibe o alerta
            }
            ...
            if (sucesso) {
                String erroSetores = salvarConfiguracaoSetores();
                String erroParticipantes = salvarConfiguracaoParticipantes();
                
                if (erroSetores != null || erroParticipantes != null) {
                    String mensagemErro = "Inventário salvo, mas houve erro ao configurar:\n";
                    if (erroSetores != null) mensagemErro += "- Setores: " + erroSetores + "\n";
                    if (erroParticipantes != null) mensagemErro += "- Participantes: " + erroParticipantes + "\n";
                    mensagemErro += "\nAs restrições de permissões ou dados bloqueiam a alteração!";
                    
                    JOptionPane.showMessageDialog(this, mensagemErro, "Aviso de Falha", JOptionPane.WARNING_MESSAGE);
                } else {
    ...
    /**
     * Valida os setores selecionados antes de salvar.
     */
    private boolean validarConfiguracaoSetores() {
        boolean incluirTodos = checkIncluirTodos.isSelected();
        if (!incluirTodos) {
            ...
            if (setoresSelecionados.isEmpty()) {
                JOptionPane.showMessageDialog(this, 
                    ...
                    "Validação de Setores", JOptionPane.WARNING_MESSAGE);
                return false;
            }
        }
        return true;
     ...
     * @return null se sucesso, ou mensagem de erro se falha
     */
    private String salvarConfiguracaoSetores() {
        try {
            boolean incluirTodos = checkIncluirTodos.isSelected();
            ...
            if (!sucesso) {
                return "Falha interna ao salvar (DAO retornou false)";
            }
            return null;
            
        } catch (Exception e) {
            e.printStackTrace();
            return "Erro ao salvar setores: " + e.getMessage();
        }
    }
    ...
    /**
     * Valida os participantes selecionados antes de salvar.
     */
    private boolean validarConfiguracaoParticipantes() {
        boolean incluirTodos = checkIncluirTodosUsuarios.isSelected();
        if (!incluirTodos) {
            if (modeloParticipantesSelecionados.isEmpty()) {
                JOptionPane.showMessageDialog(this, 
                    "Adicione pelo menos um participante ou marque 'Incluir todos os usuários ativos'.", 
                    "Validação de Participantes", JOptionPane.WARNING_MESSAGE);
                return false;
            }
        }
        return true;
     ...
     * @return null se sucesso, ou mensagem de erro se falha
     */
    private String salvarConfiguracaoParticipantes() {
        try {
            boolean incluirTodos = checkIncluirTodosUsuarios.isSelected();
            ...
            e.printStackTrace();
            return "Erro ao salvar participantes: " + e.getMessage();
        }
    }
}