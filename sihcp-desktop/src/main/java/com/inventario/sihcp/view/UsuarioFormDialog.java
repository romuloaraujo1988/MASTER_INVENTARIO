...
import com.inventario.sihcp.util.PasswordUtil;
...
public class UsuarioFormDialog extends JDialog {
    ...
    private void initializeComponents() {
        ...
        // Botões
        btnSalvar = ButtonStyleFactory.createPrimaryButton("Salvar");
    ...
    private void setupLayout() {
        ...
        gbc.gridx = 1; gbc.fill = GridBagConstraints.HORIZONTAL;
        cbSetor.setPreferredSize(new Dimension(10, 30));
        ...
        gbc.gridx = 1; gbc.fill = GridBagConstraints.BOTH; gbc.weighty = 0.0;
    ...
    private void carregarSetores() {
        try {
            ...
            if (com.inventario.sihcp.offline.OfflineModeManager.getInstance().isModoOffline()) {
                // Carregar do OfflineDAO
                var setoresMap = new com.inventario.sihcp.offline.OfflineDAO().listarSetores();
                setores = new java.util.ArrayList<>();
                for (var map : setoresMap) {
                    Setor s = new Setor();
                    s.setId((Integer) map.get("id"));
                    s.setNome((String) map.get("nome"));
                    s.setDescricao((String) map.get("descricao"));
                    setores.add(s);
                }
            } else {
                setores = setorDAO.findAll("NOME");
            }
