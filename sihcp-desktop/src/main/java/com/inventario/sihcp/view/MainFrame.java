...
import com.inventario.sihcp.offline.StatusBarPanel;
import com.inventario.sihcp.offline.SyncStatusManager;
...
// import com.inventario.sihcp.MobileApiApplication;
...
public class MainFrame extends JFrame {
    ...
    public MainFrame(Usuario usuarioLogado) {
        ...
        // Inicializar o OfflineManager se ainda não foi inicializado
        try {
            if (offlineManager.getCurrentState() == OfflineManager.OfflineState.INITIALIZING) {
                offlineManager.initialize();
            }
        } catch (SQLException e) {
            System.err.println("Erro ao inicializar OfflineManager: " + e.getMessage());
            // Continuar mesmo com erro - o sistema pode operar sem modo offline
        }
    ...
    private JPanel createDashboardPanel() {
        ...
        if (isConsulta) {
            ...
            // Usar o novo WrapLayout para ser responsivo
            com.inventario.sihcp.util.WrapLayout wrapLayout = new com.inventario.sihcp.util.WrapLayout(FlowLayout.CENTER, 20, 20);
            buttonContainer.setLayout(wrapLayout);
            ...
            // Botão Patrimônios
            JButton btnPatrimonios = createSimpleButton("Patrimônios", "🏷");
            btnPatrimonios.addActionListener(e -> abrirPatrimonio());
            buttonContainer.add(btnPatrimonios);
            ...
            gbc.weightx = 1.0;
            gbc.weighty = 1.0;
            gbc.fill = GridBagConstraints.HORIZONTAL; // Permite esticar horizontalmente para o WrapLayout usar o espaço
            gbc.anchor = GridBagConstraints.NORTH; // Alinhar ao topo após o título
            panel.add(buttonContainer, gbc);
        }
    ...
    private JButton createSimpleButton(String text, String icon) {
        JButton button = new JButton() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                // Desenhar ícone e texto dinamicamente baseados no tamanho do botão
                ...
                // Calcular tamanho dinâmico (responsivo)
                int minDim = Math.min(getWidth(), getHeight());
                int iconSize = Math.max(24, minDim / 4); // Ícone cresce junto com o botão
                int textSize = Math.max(14, minDim / 10); // Texto cresce levemente
                
                // Posições baseadas no tamanho
                int totalContentHeight = iconSize + textSize + 15; // Altura aproximada do conteúdo
                int startY = (getHeight() - totalContentHeight) / 2 + (iconSize / 2);

                // Desenhar ícone
                if (icon != null && !icon.isEmpty()) {
                    g2d.setFont(new Font("Segoe UI Symbol", Font.PLAIN, iconSize));
                    g2d.setColor(Color.WHITE);
                    FontMetrics fmIcon = g2d.getFontMetrics();
                    int iconWidth = fmIcon.stringWidth(icon);
                    int iconX = centerX - (iconWidth / 2);
                    // O Y drawString no Java é o baseline
                    g2d.drawString(icon, iconX, startY);
                    startY += iconSize/2 + 15;
                }

                // Desenhar texto
                g2d.setFont(new Font("Segoe UI", Font.BOLD, textSize));
        ...
        // Tamanhos controlados - o tamanho padrão agora rege o WrapLayout
        button.setPreferredSize(new Dimension(200, 150));
        button.setMinimumSize(new Dimension(200, 150));
    ...
    private void updateConnectionStatus(OfflineManager.OfflineState state) {
        // Atualizar o novo painel de status (que é o visível ao usuário)
        if (statusBarPanel != null) {
            statusBarPanel.atualizarStatus(state);
        }

        // Legado: atualizar labels e botões antigos se existirem no layout
        ...
        if (btnModoOffline != null) btnModoOffline.setEnabled(state == OfflineManager.OfflineState.ONLINE);
        if (btnTentarOnline != null) btnTentarOnline.setEnabled(state == OfflineManager.OfflineState.OFFLINE || state == OfflineManager.OfflineState.ERROR);
    }
    ...
    private void abrirExportacaoSiads() {
        try {
            com.inventario.sihcp.siads.view.SiadsExportDialog dialog = new com.inventario.sihcp.siads.view.SiadsExportDialog(this);
    ...
    private void abrirAnalyticsDashboard() {
        try {
            com.inventario.sihcp.analytics.view.AnalyticsDashboardFrame analyticsFrame = 
                new com.inventario.sihcp.analytics.view.AnalyticsDashboardFrame();
    ...
    private void abrirDivergenciasAnalytics() {
        try {
            ...
            com.inventario.sihcp.analytics.view.DivergenciasAnalyticsFrame frame = 
                new com.inventario.sihcp.analytics.view.DivergenciasAnalyticsFrame(inventarioAtivo.getId());
            frame.setVisible(true);
        } catch (SQLException e) {
            ModernDialog.showMessage(this,
    ...
    private void abrirMetricasTempoAnalytics() {
        try {
            ...
            com.inventario.sihcp.analytics.view.MetricasTempoAnalyticsFrame frame = 
                new com.inventario.sihcp.analytics.view.MetricasTempoAnalyticsFrame(inventarioAtivo.getId());
            frame.setVisible(true);
        } catch (SQLException e) {
            ModernDialog.showMessage(this,
    ...
    private void mostrarSobre() {
        ...
        JLabel label = new JLabel(com.inventario.sihcp.util.VersionInfo.getVersionInfoHtml());
    ...
    private void atualizarContadorColetasPendentes() {
        try {
            com.inventario.sihcp.offline.OfflineDAO offlineDAO = new com.inventario.sihcp.offline.OfflineDAO();
            
            // ✅ CORREÇÃO: Se estiver online, limpar coletas pendentes que já existem no PostgreSQL
            if (!offlineManager.isOperatingOffline()) {
                try {
                    com.inventario.sihcp.dao.ColetaDAO coletaDAO = new com.inventario.sihcp.dao.ColetaDAO();
        ...
        SwingWorker<Boolean, String> worker = new SwingWorker<Boolean, String>() {
            ...
            protected Boolean doInBackground() throws Exception {
                try {
                    ...
                    com.inventario.sihcp.offline.DataSynchronizer.SyncResult result = 
    ...
    private boolean verificarConectividadeServidor() {
        try {
            ...
            com.inventario.sihcp.offline.ConnectivityManager connManager = 
                com.inventario.sihcp.offline.ConnectivityManager.getInstance();
