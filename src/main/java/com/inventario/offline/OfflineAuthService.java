...
public class OfflineAuthService {
    ...
    private boolean verificarSenha(String senhaDigitada, String senhaHash) {
        try {
            ...
            boolean senhaCorreta = com.inventario.util.PasswordUtil.verifyPassword(senhaDigitada, senhaHash);
            
            // Se falhar, tentar verificação simples (compatibilidade)
            if (!senhaCorreta) {
                senhaCorreta = com.inventario.util.PasswordUtil.verifyPasswordSimple(senhaDigitada, senhaHash);
            }
    ...
    public boolean criarUsuarioAdminPadrao() {
        try (Connection conn = sqliteConnection.getConnection()) {
            ...
            if (admin == null) {
                ...
                admin.setSenhaHash(com.inventario.util.PasswordUtil.hashPassword("admin123"));
