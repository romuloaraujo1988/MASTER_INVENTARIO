...
import com.inventario.dao.CampusDAO;
...
public class CampusService {
    ...
    // Padrão para validação de CNPJ (formato: XX.XXX.XXX/XXXX-XX)
    ...
    // Padrão para validação de telefone (formato: (XX) XXXXX-XXXX ou (XX) XXXX-XXXX)
     ...
     * @return ID do campus inserido ou null se houve erro
     */
    public Integer inserirCampus(Campus campus) {
        if (!validarCampus(campus)) {
            return null;
        }
        
        // Verificar se já existe campus com o mesmo nome
        if (campusDAO.campusExistePorNome(campus.getNome())) {
            System.out.println("Já existe um campus com o nome: " + campus.getNome());
            return null;
        }
        ...
        if (campus.getCnpj() != null && !campus.getCnpj().trim().isEmpty()) {
            if (campusDAO.campusExistePorCnpj(campus.getCnpj())) {
                System.out.println("Já existe um campus com o CNPJ: " + campus.getCnpj());
                return null;
            }
        }
        
        return campusDAO.inserirCampus(campus);
    }
    ...
    public boolean atualizarCampus(Campus campus) {
        if (campus.getId() <= 0) {
            System.out.println("ID do campus é obrigatório para atualização");
            return false;
        }
        ...
        if (campusExistente != null && campusExistente.getId() != campus.getId()) {
            System.out.println("Já existe outro campus com o nome: " + campus.getNome());
            return false;
        }
        ...
        if (campus.getCnpj() != null && !campus.getCnpj().trim().isEmpty()) {
            ...
            if (campusCnpj != null && campusCnpj.getId() != campus.getId()) {
                System.out.println("Já existe outro campus com o CNPJ: " + campus.getCnpj());
                return false;
            }
        }
    ...
    /**
     * Valida os dados de um campus
     * @param campus Campus a ser validado
     * @return true se válido
     */
    private boolean validarCampus(Campus campus) {
        if (campus == null) {
            System.out.println("Campus não pode ser nulo");
            return false;
        }
        
        // Validar nome (obrigatório)
        if (campus.getNome() == null || campus.getNome().trim().isEmpty()) {
            System.out.println("Nome do campus é obrigatório");
            return false;
        }
        
        if (campus.getNome().trim().length() > 100) {
            System.out.println("Nome do campus não pode ter mais de 100 caracteres");
            return false;
        }
        
        // Validar local (obrigatório)
        if (campus.getLocal() == null || campus.getLocal().trim().isEmpty()) {
            System.out.println("Local do campus é obrigatório");
            return false;
        }
        
        if (campus.getLocal().trim().length() > 200) {
            System.out.println("Local do campus não pode ter mais de 200 caracteres");
            return false;
        }
        
        // Validar CNPJ (opcional, mas se informado deve ser válido)
        if (campus.getCnpj() != null && !campus.getCnpj().trim().isEmpty()) {
            if (!validarCnpj(campus.getCnpj().trim())) {
                System.out.println("CNPJ inválido: " + campus.getCnpj());
                return false;
            }
        }
        ...
        if (campus.getDiretor() != null && campus.getDiretor().trim().length() > 100) {
            System.out.println("Nome do diretor não pode ter mais de 100 caracteres");
            return false;
        }
        
        // Validar cursos (opcional)
        if (campus.getCursos() != null && campus.getCursos().trim().length() > 500) {
            System.out.println("Lista de cursos não pode ter mais de 500 caracteres");
            return false;
        }
        ...
        if (campus.getTelefone() != null && !campus.getTelefone().trim().isEmpty()) {
            if (!validarTelefone(campus.getTelefone().trim())) {
                System.out.println("Telefone inválido: " + campus.getTelefone());
                return false;
            }
        }
        ...
        if (campus.getEmail() != null && !campus.getEmail().trim().isEmpty()) {
            if (!validarEmail(campus.getEmail().trim())) {
                System.out.println("Email inválido: " + campus.getEmail());
                return false;
            }
        }
        ...
        if (campus.getObservacoes() != null && campus.getObservacoes().trim().length() > 1000) {
            System.out.println("Observações não podem ter mais de 1000 caracteres");
            return false;
        }
