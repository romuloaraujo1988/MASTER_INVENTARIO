 ...
 * Contém métodos para gerar diversos tipos de relatórios baseados nos dados de coleta.
 * 
 * IMPORTANTE: Todos os relatórios são vinculados ao inventário ativo do sistema,
...
public class RelatorioColetaDAO {
     ...
     * IMPORTANTE: Este método trabalha apenas com o inventário ativo do sistema
     * @param idInventario ID do inventário ativo
     * @return Lista de mapas com dados dos itens não encontrados
     */
    public List<Map<String, Object>> gerarRelatorioItensNaoEncontrados(int idInventario) {
        String sql = """
            SELECT 
                ...
                r.NOME as "Responsável",
                s.NOME as "Setor",
                c.LOCALIZACAO_ATUAL as "Última Localização",
                c.DATA_COLETA as "Data Tentativa",
                c.OBSERVACAO_COLETA as "Motivo",
                p.STATUS as "situacao",
                p.VALOR_AQUISICAO as "Valor"
            FROM TABELA_COLETA c
            INNER JOIN TABELA_PATRIMONIO p ON c.ID_PATRIMONIO = p.ID
            ...
            LEFT JOIN TABELA_SETOR s ON r.ID_SETOR = s.ID
            WHERE c.ID_INVENTARIO = ? 
              AND c.STATUS_COLETA = 'NAO_ENCONTRADO'
    ...
    public List<Map<String, Object>> gerarRelatorioDetalhadoPorResponsavel(int idInventario, String nomeResponsavel) {
        String sql = """
            SELECT 
                ...
                c.LOCALIZACAO_ENCONTRADA as localizacao,
        ...
        return executarConsulta(sql, idInventario, "%" + nomeResponsavel + "%");
    }
    ...
    /**
     * Busca patrimônios não coletados (que ainda não foram processados no inventário)
     * @param idInventario ID do inventário
     * @return Lista com patrimônios não coletados
     */
     ...
     */
    public List<Map<String, Object>> gerarRelatorioAvancadoPorPeriodo(int idInventario, java.util.Date dataInicio, java.util.Date dataFim) {
        String sql = """
            SELECT 
                c.DATA_COLETA as "Data Coleta",
                s.NOME as "Setor",
     ...
     */
    public List<Map<String, Object>> gerarEstatisticasAvancadasPorSetor(int idInventario, java.util.Date dataInicio, java.util.Date dataFim) {
        StringBuilder sql = new StringBuilder("""
            SELECT 
                s.NOME as "Setor",
     ...
     */
    public List<Map<String, Object>> gerarRelatorioConsolidado(int idInventario, java.util.Date dataInicio, java.util.Date dataFim) {
        StringBuilder sql = new StringBuilder("""
            SELECT 
     ...
     * Fornece uma visão abrangente de todo o inventário incluindo itens coletados, não coletados e não encontrados
     * @param idInventario ID do inventário ativo
