...
public class JwtUtil {
    ...
    /**
     * Validação defensiva — SecretsBootstrap já valida globalmente, mas JwtUtil pode ser usado em testes isolados
     */
    @PostConstruct
    private void validateSecret() {
        if (secret == null || secret.length() < 32) {
            throw new IllegalStateException(
                "JWT_SECRET muito curto: " + (secret == null ? 0 : secret.length())
                + " chars (mínimo: 32)"
            );
        }
    }
