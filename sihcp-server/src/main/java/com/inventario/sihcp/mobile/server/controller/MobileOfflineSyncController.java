...
import com.inventario.sihcp.mobile.server.service.MobileOfflineSyncService;
...
public class MobileOfflineSyncController {
     ...
     * Endpoint LEVE — executa apenas COUNT(*), sem carregar dados completos.
     */
    @GetMapping("/status")
    @Operation(
        summary = "Verificar status do servidor",
        description = "Endpoint leve para verificar se o servidor está respondendo e quantos dados estão disponíveis. Usa apenas COUNT(*) — não carrega dados completos."
    )
    public ResponseEntity<ApiResponse<MobileOfflineDataDTO.MetadataDTO>> verificarStatus(
        ...
        try {
            ...
            // Apenas metadados via COUNT(*) — sem carregar patrimônios, salas ou responsáveis
            MobileOfflineDataDTO.MetadataDTO metadata = offlineSyncService.buscarMetadadosOffline(inventarioId);
