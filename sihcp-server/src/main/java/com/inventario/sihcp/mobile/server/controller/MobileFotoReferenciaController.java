...
import com.inventario.sihcp.mobile.server.service.MobileFotoReferenciaService;
...
public class MobileFotoReferenciaController {
    ...
    public ResponseEntity<ApiResponse<PagedResponse<MobileFotoReferenciaDTO>>> listarFotos(
        ...
        try {
            ...
            if ("42P01".equals(e.getSQLState()) || (e.getMessage() != null && e.getMessage().contains("não existe"))) {
                logger.warn("⚠️ Tabela 'tabela_foto_referencia' não existe no banco. " +
                    "O recurso de fotos de referência está desabilitado. Execute o SQL de criação da tabela. " +
                    "ERRO: {}", e.getMessage());
                // Retorna lista vazia com 200 OK para não travar o app (feature opcional)
                PagedResponse<MobileFotoReferenciaDTO> empty =
                    new PagedResponse<>(java.util.Collections.emptyList(), page, size, 0);
                return ResponseEntity.ok(
                    ApiResponse.success(empty, "Recurso de fotos de referência não disponível (tabela não existe)"));
            }
