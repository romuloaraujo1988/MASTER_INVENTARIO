...
public class DatabaseConnection {
    ...
    public static Connection getConnection() throws SQLException {
        try {
            ...
            try {
                return com.inventario.sihcp.config.HikariConnectionPool.getConnection();
            } catch (Throwable e) {
                System.err.println("⚠️ Falha ao obter conexão do pool HikariCP (caindo para fallback direto): " + e.getMessage());
    ...
    private static Connection createConnectionFromJson() throws SQLException {
        ...
        // Inicializar o pool com estes dados para as próximas chamadas
        try {
            com.inventario.sihcp.config.HikariConnectionPool.setup(url, jsonUser, jsonPassword);
        } catch (Throwable e) {
            System.err.println("Erro ao inicializar pool via JSON: " + e.getMessage());
        }
        
        Properties props = new Properties();
        props.setProperty("user", jsonUser);
        props.setProperty("password", jsonPassword);
        props.setProperty("connectTimeout", "30"); // Aumentado para 30s
        props.setProperty("socketTimeout", "600"); // Aumentado para 10 min para imports longos
    ...
    private static Connection createConnectionFromConfig(DatabaseConfig config) throws SQLException {
        ...
        // Inicializar o pool com estes dados para as próximas chamadas
        try {
            com.inventario.sihcp.config.HikariConnectionPool.setup(url, config.getUsername(), config.getPassword());
        } catch (Throwable e) {
            System.err.println("Erro ao inicializar pool via Config: " + e.getMessage());
        }
