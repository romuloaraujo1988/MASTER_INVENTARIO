...
public class DateFormatUtils {
    ...
    /**
     * Converte string para Date de forma robusta, tentando múltiplos formatos
     * @param dateString String contendo a data
     * @return Date ou null se não conseguir converter
     */
    public static Date parseAny(String dateString) {
        if (dateString == null || dateString.trim().isEmpty() || dateString.equals("-")) {
            return null;
        }
        
        String val = dateString.trim();
        
        // 1. Tentar formatos brasileiros (mais comum em planilhas)
        Date d = parseDateTimeFull(val);
        if (d != null) return d;
        ...
        // 2. Tentar formatos ISO/API
        d = parseFromApi(val);
        if (d != null) return d;
        
        d = parseDateFromApi(val);
        if (d != null) return d;
        
        // 3. Tentar via timestamp robusto
        try {
            return parseTimestampFromStringOrNull(val);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public static Timestamp parseTimestampFromString(String strValue) {
        Timestamp ts = parseTimestampFromStringOrNull(strValue);
        if (ts != null) {
            return ts;
        }
    ...
    /**
     * Converte uma string para Timestamp de forma robusta
     * Suporta múltiplos formatos de data. Retorna NULL caso não consiga converter.
     * 
     * @param strValue String contendo a data
     * @return Timestamp ou null se não conseguir converter
     */
    public static Timestamp parseTimestampFromStringOrNull(String strValue) {
