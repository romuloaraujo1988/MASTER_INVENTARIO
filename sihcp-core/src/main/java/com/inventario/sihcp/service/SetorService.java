...
public class SetorService {
    ...
    public void excluir(int id) throws BusinessException {
        try {
            ...
            if (qtdResponsaveis > 0 || qtdSalas > 0 || qtdUsuarios > 0) {
                StringBuilder msg = new StringBuilder(
                    "Não é possível excluir este setor pois possui:");
                if (qtdResponsaveis > 0)
                    msg.append("\n  - ").append(qtdResponsaveis).append(" responsável(is) vinculado(s)");
                if (qtdSalas > 0)
                    msg.append("\n  - ").append(qtdSalas).append(" sala(s) vinculada(s)");
                if (qtdUsuarios > 0)
                    msg.append("\n  - ").append(qtdUsuarios).append(" usuário(s) vinculado(s)");
                msg.append("\n\nDesvincule os registros acima antes de excluir o setor.");
