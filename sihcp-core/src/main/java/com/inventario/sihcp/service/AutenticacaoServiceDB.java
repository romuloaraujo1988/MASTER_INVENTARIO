...
public class AutenticacaoServiceDB {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AutenticacaoServiceDB.class);
    ...
    public Usuario autenticar(String login, String senha) {
        ...
        if (usuario == null) {
            logger.info("Usuário não encontrado: {}", login);
            return null;
        }
        ...
        try {
            ...
            senhaCorreta = com.inventario.sihcp.util.PasswordUtil.verifyPassword(senha, usuario.getSenhaHash());

            // Se falhar, tentar verificação simples (compatibilidade com senhas antigas)
            if (!senhaCorreta) {
                senhaCorreta = com.inventario.sihcp.util.PasswordUtil.verifyPasswordSimple(senha,
            ...
            logger.warn("Erro na verificação de senha: {}", e.getMessage());
        ...
        if (!senhaCorreta) {
            ...
            Integer tentativasAtual = usuario.getTentativasLogin();
            int tentativas = (tentativasAtual != null ? tentativasAtual : 0) + 1;
            ...
            if (tentativas >= MAX_TENTATIVAS_LOGIN) {
                ...
                try {
                    ...
                    logger.error("Erro ao bloquear usuário: {}", e.getMessage());
                }
                logger.info("Usuário bloqueado por excesso de tentativas: {}", login);
            } else {
                // Atualizar tentativas no banco
                try {
                    usuarioDAO.update(usuario);
                } catch (SQLException e) {
                    logger.error("Erro ao atualizar tentativas de login: {}", e.getMessage());
                }
                logger.info("Senha incorreta para usuário: {} (Tentativa {}/{})", login, tentativas,
        ...
        try {
            logger.info("DIAGNOSTICO: Registrando acesso para usuário {} com data: {}",
                    usuario.getLogin(), usuario.getDataUltimoAcesso());
            usuarioDAO.update(usuario);
            logger.info("DIAGNOSTICO: Update executado com sucesso para {}.", usuario.getLogin());
        } catch (SQLException e) {
            logger.error("DIAGNOSTICO: Erro ao atualizar dados do usuário {}: {}",
                    usuario.getLogin(), e.getMessage(), e);
        }
    ...
    public String gerarHashSenha(String senha) {
        try {
            return com.inventario.sihcp.util.PasswordUtil.hashPassword(senha);
        } catch (Exception e) {
            logger.error("Erro ao gerar hash da senha: {}", e.getMessage());
            // Fallback para hash simples em caso de erro
            return com.inventario.sihcp.util.PasswordUtil.hashPasswordSimple(senha);
        }
    }
    ...
    public boolean alterarSenha(int usuarioId, String senhaAtual, String novaSenha) {
        ...
        try {
            senhaAtualCorreta = com.inventario.sihcp.util.PasswordUtil.verifyPassword(senhaAtual,
                    usuario.getSenhaHash());

            // Fallback para verificação simples
            if (!senhaAtualCorreta) {
                senhaAtualCorreta = com.inventario.sihcp.util.PasswordUtil.verifyPasswordSimple(senhaAtual,
                        usuario.getSenhaHash());
            }
    ...
    public boolean desbloquearUsuario(int usuarioId) {
        try {
            ...
            logger.error("Erro ao desbloquear usuário: {}", e.getMessage());
            return false;
        }
    }
    ...
    public void criarUsuarioAdminPadrao() {
        ...
        try {
            ...
            logger.error("Erro ao buscar usuário admin: {}", e.getMessage());
            return;
        }
        ...
        if (admin == null) {
            ...
            try {
                ...
                logger.error("Erro ao criar usuário administrador padrão: {}", e.getMessage());
            }
        }
    }
}