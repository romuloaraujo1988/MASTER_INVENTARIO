...
import com.inventario.sihcp.model.InventarioSetor;
...
public class InventarioSetorDAO {
    ...
    public boolean salvar(InventarioSetor inventarioSetor) {
        ...
        try (Connection connection = getConnection();
            ...
            logger.error("Erro ao salvar InventarioSetor: {}", e.getMessage(), e);
            throw new RuntimeException("Erro ao salvar configuração de setor: " + e.getMessage(), e);
        }
    ...
    /**
     * Desativa todos os relacionamentos de um inventário (soft delete)
     */
    public boolean removerPorInventario(Integer idInventario) {
        ...
        try (Connection connection = getConnection();
            ...
            return stmt.executeUpdate() >= 0; // >= 0 because updating 0 rows is still a success if empty
