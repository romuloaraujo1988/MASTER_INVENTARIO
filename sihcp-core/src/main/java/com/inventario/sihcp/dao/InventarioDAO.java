...
public class InventarioDAO extends BaseDAO<Inventario, Integer> {
    ...
    protected String getInsertSQL() {
        return "INSERT INTO TABELA_INVENTARIO (NOME, ANO, DATA_INICIO, DATA_FIM, STATUS_INVENTARIO, " +
                "RESPONSAVEL_INVENTARIO, PERCENTUAL_CONCLUSAO) VALUES (?, ?, ?, ?, ?, ?, ?)";
    }

    @Override
    protected String getUpdateSQL() {
        return "UPDATE TABELA_INVENTARIO SET NOME = ?, ANO = ?, DATA_INICIO = ?, DATA_FIM = ?, " +
    ...
    protected void setInsertParameters(PreparedStatement stmt, Inventario inventario) throws SQLException {
        ...
        // Extrair o ano da data de início caso o ano não esteja definido
        if (inventario.getAno() == null && inventario.getDataInicio() != null) {
            java.util.Calendar cal = java.util.Calendar.getInstance();
            cal.setTime(inventario.getDataInicio());
            inventario.setAno(cal.get(java.util.Calendar.YEAR));
        }
        
        if (inventario.getAno() != null) {
            stmt.setInt(2, inventario.getAno());
        } else {
            java.util.Calendar cal = java.util.Calendar.getInstance();
        ...
        if (inventario.getDataInicio() != null) {
            stmt.setDate(3, new java.sql.Date(inventario.getDataInicio().getTime()));
        } else {
            stmt.setNull(3, java.sql.Types.DATE);
        }
        
        if (inventario.getDataFim() != null) {
            stmt.setDate(4, new java.sql.Date(inventario.getDataFim().getTime()));
        } else {
            stmt.setNull(4, java.sql.Types.DATE);
        }
        
        stmt.setString(5, inventario.getStatusInventario());
        stmt.setString(6, inventario.getResponsavelInventario());
        stmt.setBigDecimal(7, inventario.getPercentualConclusao());
    }
    ...
    protected void setUpdateParameters(PreparedStatement stmt, Inventario inventario) throws SQLException {
        ...
        stmt.setInt(8, inventario.getId());
    }
    ...
    protected Inventario mapResultSetToEntity(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            if (!rs.wasNull()) {
                inventario.setAno(ano);
            }
        } catch (SQLException e) {
            // Ignorar se a coluna não existir para retrocompatibilidade
        }
