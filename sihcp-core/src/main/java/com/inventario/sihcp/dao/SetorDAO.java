...
public class SetorDAO extends BaseDAO<Setor, Integer> {
    ...
    public Setor buscarPorNome(String nome) throws SQLException {
        String sql = "SELECT * FROM TABELA_SETOR WHERE NOME ILIKE ?";
        return executeQuerySingle(sql, nome);
    }
    
    /**
     * Busca setor por nome (usando conexão existente)
     */
    public Setor buscarPorNome(String nome, Connection conn) throws SQLException {
        // Usar ILIKE para busca exata mas case-insensitive (PostgreSQL)
        String sql = "SELECT * FROM TABELA_SETOR WHERE NOME ILIKE ?";
        return executeQuerySingle(conn, sql, nome);
    }
    ...
    /**
     * Conta quantos usuários estão vinculados ao setor (independente de estarem ativos)
     * Necessário para validar exclusão física
     */
    public int contarUsuariosVinculados(int idSetor) throws SQLException {
        String sql = "SELECT COUNT(*)::INTEGER FROM TABELA_USUARIO WHERE ID_SETOR = ?";
        Integer count = executeScalar(sql, Integer.class, idSetor);
        return count != null ? count : 0;
    }

    /**
     * Conta quantos coletores estão vinculados ao setor
     */
    public int contarColetoresVinculados(int idSetor) throws SQLException {
        String sql = "SELECT COUNT(*)::INTEGER FROM TABELA_COLETOR WHERE ID_SETOR = ?";
        Integer count = executeScalar(sql, Integer.class, idSetor);
        return count != null ? count : 0;
    }

    /**
     * Conta quantos inventários possuem este setor configurado no escopo
     */
    public int contarInventariosVinculados(int idSetor) throws SQLException {
        String sql = "SELECT COUNT(*)::INTEGER FROM TABELA_INVENTARIO_SETOR WHERE ID_SETOR = ?";
        Integer count = executeScalar(sql, Integer.class, idSetor);
        return count != null ? count : 0;
    }
    ...
    /**
     * Busca setores por termo (nome ou descrição) usando conexão existente
     */
    public List<Setor> buscarPorTermo(String termo, Connection conn) throws SQLException {
        String sql = "SELECT * FROM TABELA_SETOR " +
                    "WHERE NOME ILIKE ? OR DESCRICAO ILIKE ? " +
                    "ORDER BY NOME";
        String termoBusca = "%" + termo + "%";
        return executeQuery(conn, sql, termoBusca, termoBusca);
    }
