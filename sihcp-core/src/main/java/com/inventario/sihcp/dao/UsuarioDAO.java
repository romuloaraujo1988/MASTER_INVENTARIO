...
public class UsuarioDAO extends BaseDAO<Usuario, Integer> {
    ...
    protected String getUpdateSQL() {
        return "UPDATE TABELA_USUARIO SET LOGIN = ?, SENHA_HASH = ?, NOME_COMPLETO = ?, " +
               ...
               "PRIMEIRO_ACESSO = ?, OBSERVACOES = ?, DATA_ULTIMO_ACESSO = ?, TENTATIVAS_LOGIN = ?, " +
    ...
    protected void setInsertParameters(PreparedStatement stmt, Usuario usuario) throws SQLException {
        ...
        // Campos BOOLEAN - usar setBoolean para compatibilidade com PostgreSQL
    ...
    protected void setUpdateParameters(PreparedStatement stmt, Usuario usuario) throws SQLException {
        ...
        // DATA_ULTIMO_ACESSO - pode ser null
        if (usuario.getDataUltimoAcesso() != null) {
            logger.info("DIAGNOSTICO: Gravando DATA_ULTIMO_ACESSO no banco para {}: {}", usuario.getLogin(), usuario.getDataUltimoAcesso());
            stmt.setTimestamp(12, Timestamp.valueOf(usuario.getDataUltimoAcesso()));
        } else {
            logger.warn("DIAGNOSTICO: Gravando DATA_ULTIMO_ACESSO no banco para {}: NULL", usuario.getLogin());
            stmt.setNull(12, java.sql.Types.TIMESTAMP);
        }
        
        // TENTATIVAS_LOGIN - pode ser null
        if (usuario.getTentativasLogin() != null) {
            stmt.setInt(13, usuario.getTentativasLogin());
        } else {
            stmt.setInt(13, 0);
        }
    ...
    /**
     * Verifica se o usuário possui vínculo com algum inventário
     * (participante registrado, ativo ou inativo).
     *
     * @param id ID do usuário
     * @return true se existir qualquer registro em tabela_participante_inventario
     */
    public boolean temVinculoComInventario(int id) throws SQLException {
        String sql = "SELECT COUNT(*) FROM tabela_participante_inventario WHERE id_usuario = ?";
        Long count = executeScalar(sql, Long.class, id);
        return count != null && count > 0;
    }

    /**
     * Exclusão física do usuário.
     * Deve ser chamado APENAS quando temVinculoComInventario() retornar false.
     *
     * @param id ID do usuário a excluir permanentemente
     */
    public void deleteFisico(int id) throws SQLException {
        String sql = "DELETE FROM TABELA_USUARIO WHERE ID = ?";
        executeUpdate(sql, id);
    }
    ...
    public List<Usuario> buscarPorIds(List<Integer> ids) throws SQLException {
        ...
        try {
            conn = com.inventario.sihcp.util.DatabaseConnection.getConnection();
