...
public class ItemCompostoDAO {
            ...
            sql = """
                INSERT INTO tabela_coleta_componente 
                    (id_item_composto, id_inventario, id_coletor, quantidade_encontrada, 
                     status_componente, observacao_coleta, localizacao_encontrada, data_coleta)
    ...
    /**
     * Busca o histórico de coletas de componentes de um inventário
     * 
     * @param idInventario ID do inventário
     * @return Lista de coletas com detalhes
     */
    public List<Map<String, Object>> buscarHistoricoColetas(int idInventario) throws SQLException {
        String sql = """
            SELECT 
                cc.data_coleta,
            ...
            ORDER BY cc.data_coleta DESC NULLS LAST, p.numero
            """;
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    Map<String, Object> row = new HashMap<>();
                    row.put("dataColeta", rs.getTimestamp("data_coleta"));
                    row.put("numeroPatrimonio", rs.getString("numero_patrimonio"));
                    row.put("descricaoPatrimonio", rs.getString("descricao_patrimonio"));
                    row.put("tipoComponente", rs.getString("tipo_componente"));
                    row.put("quantidadeEncontrada", rs.getInt("quantidade_encontrada"));
                    row.put("statusComponente", rs.getString("status_componente"));
                    row.put("nomeColetor", rs.getString("nome_coletor"));
                    resultado.add(row);
                }
            }
        }
