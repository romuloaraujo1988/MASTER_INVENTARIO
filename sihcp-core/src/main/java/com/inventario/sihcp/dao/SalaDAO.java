 ...
 */
@Repository
public class SalaDAO extends BaseDAO<Sala, Integer> {
    ...
    /**
     * Busca salas por filtros específicos (usado na importação, com conexão transacional)
     */
    public List<Sala> buscarPorFiltro(String descricao, String numeroSala, String bloco, 
                                      Integer andar, Integer idSetor, String tipoSala, Boolean ativo, Connection conn) throws SQLException {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT s.*, st.NOME as NOME_SETOR ");
        sql.append("FROM TABELA_SALA s ");
        sql.append("LEFT JOIN TABELA_SETOR st ON s.ID_SETOR = st.ID ");
        ...
        return executeQuery(conn, sql.toString(), parametros.toArray());
    }
    ...
    public List<Sala> listarSalasComContagemPatrimonios() throws SQLException {
        String sql = """
            ...
            FROM TABELA_SALA s
            LEFT JOIN TABELA_SETOR st ON s.ID_SETOR = st.ID
            LEFT JOIN TABELA_PATRIMONIO p ON p.ID_SALA = s.ID_SALA AND (p.STATUS IS NULL OR UPPER(p.STATUS) != 'BAIXADO' OR p.STATUS = '')
            WHERE s.ATIVO = TRUE
