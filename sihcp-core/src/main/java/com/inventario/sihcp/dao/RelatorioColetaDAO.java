...
public class RelatorioColetaDAO {
    ...
    public List<Map<String, Object>> gerarRelatorioItensEncontrados(int idInventario) {
        String sql = """
                SELECT
                    ...
                    p.MARCA as "marca",
                    p.MODELO as "modelo",
                    p.NUMERO_NOTA_FISCAL as "Nota Fiscal",
    ...
    public List<Map<String, Object>> gerarRelatorioItensNaoEncontrados(int idInventario) {
        System.out.println("\n=== RELATÓRIO ITENS NÃO ENCONTRADOS (COM ITENS COMPOSTOS) ===");
        System.out.println("📋 Inventário ID: " + idInventario);

        // Query CORRIGIDA para considerar itens compostos:
        // Um patrimônio é considerado "coletado" se:
        // 1. Foi coletado diretamente na TABELA_COLETA, OU
        // 2. É um item composto e todos os componentes obrigatórios foram coletados
        // CORREÇÃO 11/02/2026: Integração com itens compostos
        String sql = """
                WITH itens_compostos_coletados AS (
                    -- Itens compostos onde TODOS os componentes obrigatórios foram coletados
                    SELECT DISTINCT ic.ID_PATRIMONIO_PRINCIPAL
                    FROM TABELA_ITEM_COMPOSTO ic
                    WHERE ic.OBRIGATORIO = true
                    GROUP BY ic.ID_PATRIMONIO_PRINCIPAL
                    HAVING COUNT(*) = (
                        SELECT COUNT(*)
                        FROM TABELA_COLETA_COMPONENTE cc
                        WHERE cc.ID_ITEM_COMPOSTO IN (
                            SELECT ic2.ID
                            FROM TABELA_ITEM_COMPOSTO ic2
                            WHERE ic2.ID_PATRIMONIO_PRINCIPAL = ic.ID_PATRIMONIO_PRINCIPAL
                              AND ic2.OBRIGATORIO = true
                        )
                        AND cc.ID_INVENTARIO = ?
                        AND cc.QUANTIDADE_ENCONTRADA >= (
                            SELECT ic3.QUANTIDADE_ESPERADA
                            FROM TABELA_ITEM_COMPOSTO ic3
                            WHERE ic3.ID = cc.ID_ITEM_COMPOSTO
                        )
                    )
                )
                SELECT
                    ...
                    CASE 
                        WHEN EXISTS (
                            SELECT 1 FROM TABELA_ITEM_COMPOSTO ic WHERE ic.ID_PATRIMONIO_PRINCIPAL = p.ID
                        ) THEN 'Item composto não coletado'
                        ELSE 'Não coletado'
                    END as "Situação",
                ...
                WHERE p.STATUS = 'ATIVO'
                  ...
                  -- E não está na lista de itens compostos totalmente coletados
                  AND p.ID NOT IN (SELECT ID_PATRIMONIO_PRINCIPAL FROM itens_compostos_coletados)
                ORDER BY
        ...
        System.out.println("🔍 Query COM ITENS COMPOSTOS:");
        System.out.println("   - Patrimônios ATIVOS que NÃO estão na TABELA_COLETA");
        System.out.println("   - E que NÃO têm todos os componentes obrigatórios coletados");

        List<Map<String, Object>> resultado = executarConsulta(sql, idInventario, idInventario);
        ...
        if (resultado.isEmpty()) {
            ...
            System.out.println("   (Incluindo itens compostos com componentes coletados)");
        } else {
    ...
    public List<Map<String, Object>> gerarRelatorioItensNaoColetados(int idInventario) {
            ...
            try (PreparedStatement ps = conn
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        if (count > 0) {
                            System.out.println("✅ Total com STATUS = 'ATIVO' (confirmado): " + count);
                        }
                    }
                }
            }
        ...
        // 
        // INTEGRAÇÃO 13/01/2026: Excluir patrimônios que são itens compostos
        // e já tiveram componentes coletados neste inventário
        String sql = """
                SELECT
                    p.NUMERO as "Número Patrimônio",
                    p.DESCRICAO as "Descrição",
    ...
    public List<Map<String, Object>> gerarRelatorioDetalhadoPorResponsavel(int idInventario, String nomeResponsavel,
        ...
        StringBuilder sql = new StringBuilder("""
                SELECT
                    ...
                    p.MARCA as marca,
                    p.MODELO as modelo,
                    p.NUMERO_NOTA_FISCAL as nota_fiscal,
    ...
    public List<Map<String, Object>> gerarRelatorioDivergenciasAutomaticas(int idInventario) {
        String sql = """
                SELECT
                    p.NUMERO as "Número Patrimônio",
                    p.DESCRICAO as "Descrição",
                    COALESCE(s.descricao, 'Não informado') as "Localização Cadastrada",
                    COALESCE(c.LOCALIZACAO_ENCONTRADA, 'Não informado') as "Localização Encontrada",
                    p.estado_conservacao as "Estado Cadastrado",
                    c.ESTADO_ENCONTRADO as "Estado Encontrado",
                    CASE
                    ...
                    CASE
                        WHEN c.LOCALIZACAO_ENCONTRADA IS NOT NULL 
                             AND s.descricao IS NOT NULL
                             AND UPPER(s.descricao) NOT LIKE '%' || UPPER(c.LOCALIZACAO_ENCONTRADA) || '%'
                             AND UPPER(c.LOCALIZACAO_ENCONTRADA) NOT LIKE '%' || UPPER(s.descricao) || '%'
                             THEN 'Localização'
                        WHEN c.ESTADO_ENCONTRADO IS NOT NULL AND UPPER(COALESCE(p.estado_conservacao,'')) <> UPPER(c.ESTADO_ENCONTRADO) THEN 'Estado'
                ...
                WHERE c.ID_INVENTARIO = ?
                  AND (
                      (c.LOCALIZACAO_ENCONTRADA IS NOT NULL 
                       AND s.descricao IS NOT NULL
                       AND UPPER(s.descricao) NOT LIKE '%' || UPPER(c.LOCALIZACAO_ENCONTRADA) || '%'
                       AND UPPER(c.LOCALIZACAO_ENCONTRADA) NOT LIKE '%' || UPPER(s.descricao) || '%') OR
                      (c.ESTADO_ENCONTRADO IS NOT NULL AND UPPER(COALESCE(p.estado_conservacao,'')) <> UPPER(c.ESTADO_ENCONTRADO)) OR
                      c.DIVERGENCIA = TRUE
                  )
                ORDER BY p.NUMERO
                """;
