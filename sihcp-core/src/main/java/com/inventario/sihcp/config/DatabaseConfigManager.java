...
import com.inventario.sihcp.util.PasswordEncryption;
...
public class DatabaseConfigManager {
    ...
    private void tryLoadPasswordFromJsonConfig() {
        ...
        String[] possiblePaths = {
            "config/configuracao_banco.json",
            "configuracao_banco.json",
            System.getProperty("user.dir") + File.separator + "config" + File.separator + "configuracao_banco.json",
    ...
    private void tryLoadFromJsonConfig() {
        File jsonFile = new File("config/configuracao_banco.json");
        if (!jsonFile.exists()) {
            jsonFile = new File("configuracao_banco.json");
        }
        if (!jsonFile.exists()) {
    ...
    /**
     * Lê a seção {@code campus} do {@code configuracao_banco.json} e retorna um
     * {@link CampusConfig} populado.
     *
     * <p>Compatibilidade retroativa: se o arquivo não existir ou não contiver a
     * seção {@code campus}, retorna um {@code CampusConfig} com todos os campos
     * nulos (sem lançar exceção), garantindo que instalações existentes continuem
     * funcionando normalmente.</p>
     *
     * @return {@link CampusConfig} com os dados do campus, ou objeto com campos
     *         nulos se a seção não estiver presente
     */
    public CampusConfig getCampusConfig() {
        File jsonFile = findJsonConfigFile();

        if (jsonFile == null) {
            System.out.println("DEBUG DatabaseConfigManager.getCampusConfig: "
                    + "configuracao_banco.json não encontrado — retornando CampusConfig vazio.");
            return new CampusConfig();
        }

        try {
            ObjectMapper mapper = new ObjectMapper();
            JsonNode root = mapper.readTree(jsonFile);
            JsonNode campusNode = root.get("campus");

            if (campusNode == null || campusNode.isNull()) {
                System.out.println("DEBUG DatabaseConfigManager.getCampusConfig: "
                        + "Seção 'campus' ausente em " + jsonFile.getAbsolutePath()
                        + " — retornando CampusConfig vazio.");
                return new CampusConfig();
            }

            CampusConfig config = mapper.treeToValue(campusNode, CampusConfig.class);
            System.out.println("DEBUG DatabaseConfigManager.getCampusConfig: "
                    + "Campus carregado: " + config);
            return config;

        } catch (Exception e) {
            System.err.println("DEBUG DatabaseConfigManager.getCampusConfig: "
                    + "Erro ao ler seção 'campus': " + e.getMessage());
            return new CampusConfig();
        }
    }

    /**
     * Localiza o arquivo {@code configuracao_banco.json} nos caminhos conhecidos.
     *
     * <p>Ordem de busca:</p>
     * <ol>
     *   <li>Diretório de configuração atual ({@link ConfigurationPaths#getConfigDirectory()})</li>
     *   <li>Diretório de trabalho atual ({@code user.dir})</li>
     *   <li>Diretório home do usuário ({@code user.home})</li>
     * </ol>
     *
     * @return o primeiro {@link File} encontrado, ou {@code null} se nenhum existir
     */
    private File findJsonConfigFile() {
        String[] candidates = {
            ConfigurationPaths.getConfigDirectory() + File.separator + "configuracao_banco.json",
            System.getProperty("user.dir") + File.separator + "configuracao_banco.json",
            System.getProperty("user.dir") + File.separator + "config"
                    + File.separator + "configuracao_banco.json",
            System.getProperty("user.home") + File.separator + "configuracao_banco.json"
        };

        for (String path : candidates) {
            File f = new File(path);
            if (f.exists() && f.isFile()) {
                System.out.println("DEBUG DatabaseConfigManager.findJsonConfigFile: "
                        + "Encontrado em: " + f.getAbsolutePath());
                return f;
            }
        }
        return null;
