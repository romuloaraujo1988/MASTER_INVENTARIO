...
### 🔧 Refatoração - Renomeação de Pacotes Java

#### Mudança de Pacote Raiz
- Pacote raiz Java renomeado de `com.inventario` para `com.inventario.sihcp`
- Alinha a identidade do pacote ao nome oficial do sistema (SIHCP)
- Abrange os três módulos Maven: `sihcp-core`, `sihcp-desktop`, `sihcp-server`
- **398 arquivos `.java` atualizados** (declarações de pacote, imports e FQNs)
- **6 arquivos de configuração** Spring Boot atualizados (propriedades de logging)
- **4 arquivos `pom.xml`** atualizados (groupId, parent, dependências internas, mainClass)
- App Android (`InventarioMobile/` — pacote `com.ifmt.inventariomobile`) **não alterado**

#### Classes de Entrada Atualizadas
